...
class BrowserSessionManager {
     ...
     */
    async navigateWithLock(url, options) {
        if (!this.page) {
     ...
     */
    async isCurrentlyLoggedIn(verbose = true) {
        if (!this.page)
            return false;
    ...
    async ensureLogin() {
        ...
        if (BrowserSessionManager.isLocked()) {
            ...
            await BrowserSessionManager.acquireLock();
        ...
        try {
            ...
            const isLoggedAfterLock = await this.isCurrentlyLoggedIn();
    ...
    async extractTableData(tableName) {
        ...
        try {
            ...
            // Extrair headers
            const headers = await this.page.$$eval('table.t-fancy-table thead th', ths => ths.map(th => { var _a; return ((_a = th.textContent) === null || _a === void 0 ? void 0 : _a.trim()) || ''; }));
            ...
            let rows = await this.page.$$eval('table.t-fancy-table tbody tr:not(.odd):not(.even)', trs => trs.map(tr => {
            ...
            if (rows.length === 0) {
                rows = await this.page.$$eval('table.t-fancy-table tbody tr', trs => trs.map(tr => {
                    const tds = tr.querySelectorAll('td');
                    return Array.from(tds).map(td => { var _a; return ((_a = td.textContent) === null || _a === void 0 ? void 0 : _a.trim()) || ''; });
                }));
            }
    ...
    async waitForManualLogin(timeout = 300000) {
        ...
        while (Date.now() - startTime < timeout) {
            ...
            const isLoggedIn = await this.isCurrentlyLoggedIn(false);
            if (isLoggedIn) {
     ...
     */
    async ensureLoginWithCaptchaHandling() {
        try {
