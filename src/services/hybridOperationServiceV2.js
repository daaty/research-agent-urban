...
class HybridOperationService {
    constructor(config) {
        ...
        this.isProcessing = false; // 🔒 LOCK para evitar múltiplos processamentos
    ...
    startAutoFeed() {
        this.autoFeedInterval = setInterval(async () => {
            ...
            if (this.isProcessing || this.isLoadingIds) {
                logger_1.logger.debug('HYBRID', 'Auto-feed: aguardando processamento terminar...');
                return;
            }
            try {
    ...
    startOperationLoop() {
        this.operationInterval = setInterval(async () => {
            ...
            if (this.isProcessing) {
                logger_1.logger.debug('HYBRID', 'Ciclo já em processamento, aguardando...');
                return;
            }
            try {
     ...
     */
    async simulateRechargeProcess(recharge) {
        try {
            ...
            const success = await this.dashboardScraper.processRecharge(recharge.driverId, recharge.amount);
            if (!success) {
     ...
     */
    async extractDriverPersonalData(driverId) {
        var _a, _b;
        try {
     ...
     */
    async savePersonalData(driverId, data) {
        // Simular salvamento
