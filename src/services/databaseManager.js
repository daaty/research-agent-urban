...
class DatabaseManager {
     ...
     */
    async insertDriverData(records) {
        if (!this.pool || !this.isConnected) {
            throw new Error('Banco de dados não conectado');
        }
        const client = await this.pool.connect();
     ...
     */
    async insertDriverPersonalDetails(record) {
        if (!this.pool || !this.isConnected) {
            throw new Error('Banco de dados não conectado');
        }
        const client = await this.pool.connect();
     ...
     */
    async saveDriverPerformanceData(performanceData) {
        if (!this.pool || !this.isConnected) {
            throw new Error('Banco de dados não conectado');
        }
     ...
     */
    async createScrapingSession(session) {
        if (!this.pool || !this.isConnected) {
            throw new Error('Banco de dados não conectado');
        }
        const query = `
     ...
     */
    async finishScrapingSession(sessionId) {
        if (!this.pool || !this.isConnected) {
            throw new Error('Banco de dados não conectado');
        }
        const query = `
     ...
     */
    async getRidesByDateRange(startDate, endDate, tableName) {
        if (!this.pool || !this.isConnected) {
            throw new Error('Banco de dados não conectado');
        }
        let query = `
     ...
     */
    async driverPersonalDetailsExists(driverId) {
        if (!this.pool || !this.isConnected) {
            return false;
        }
        try {
     ...
     */
    async getDriverPersonalDetails(driverId) {
        if (!this.pool || !this.isConnected) {
            return null;
        }
        try {
     ...
     */
    async getAllDriversPersonalDetails(limit = 100) {
        if (!this.pool || !this.isConnected) {
            return [];
        }
        try {
    ...
    async getDatabaseStats() {
      ...
      `;
        ...
        ORDER BY count DESC
      `;
            const [totalRecords, totalDrivers, totalPersonalDetails, totalSessions, lastScraping, lastDriversScraping, lastPersonalDetails, tableStats, driversStats, personalDetailsCityStats] = await Promise.all([
