...
class BrowserSessionManager {
    ...
    isCurrentlyLoggedIn() {
        return __awaiter(this, arguments, void 0, function* (verbose = true) {
            ...
            try {
                ...
                // Se não está nem em login nem em dashboard, assumir não logado
     ...
     */
    waitForManualLogin() {
        return __awaiter(this, arguments, void 0, function* (timeoutMs = 600000) {
            ...
            while ((Date.now() - startTime) < maxWaitTime) {
                try {
                    ...
                    if (currentUrl.includes('login')) {
                        ...
                        const captchaStillPresent = yield this.checkForCaptcha();
                        if (!captchaStillPresent) {
                            ...
                            try {
                                ...
                                if (!newUrl.includes('login')) {
                                    ...
                                    const finalCheck = yield this.isCurrentlyLoggedIn(true);
                                    if (finalCheck)
                                        return true;
                                }
                            }
                    ...
                    const isLoggedIn = yield this.isCurrentlyLoggedIn(false); // Modo não-verbose para ser mais rápido
                    ...
                    try {
                        yield this.page.evaluate(() => document.title);
                    }
                    catch (pageError) {
                        console.log('❌ Página inativa, tentando recarregar...');
                        yield this.page.reload({ waitUntil: 'domcontentloaded' });
                    }
                    yield this.page.waitForTimeout(checkInterval);
                }
            }
    ...
    ensureLoginWithCaptchaHandling() {
        return __awaiter(this, arguments, void 0, function* (skipIfLoggedIn = false) {
            try {
                ...
                yield this.page.waitForTimeout(8000); // 🔧 VNC: mais tempo inicial
                ...
                try {
                    ...
                    yield this.page.waitForSelector('#exampleInputEmail1', { timeout: 30000 });
                ...
                yield this.page.waitForTimeout(5000); // 🔧 VNC: 5s ao invés de 2s
