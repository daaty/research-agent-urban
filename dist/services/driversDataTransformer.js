...
class DriversDataTransformer {
     ...
     */
    extractAdditionalData(row, headers, tableName) {
     ...
     */
    generateDriverUniqueId(driverInfo, tableName) {
        const idComponents = [];
        ...
        if (idComponents.length === 0) {
            ...
            const fallbackHash = (0, crypto_1.createHash)('md5').update(fallbackData).digest('hex').substring(0, 8);
        ...
        if (baseUniqueId.length > 200) {
            const idHash = (0, crypto_1.createHash)('md5').update(baseUniqueId).digest('hex');
     ...
     */
    generateDataHash(uniqueId, row, tableName) {
    ...
    transformAndSave(scrapingData_1, sessionInfo_1) {
        return __awaiter(this, arguments, void 0, function* (scrapingData, sessionInfo, executionSource = 'drivers-persistent-scraper', hasChanges = true) {
            ...
            // Salvar no banco
            yield this.saveToDatabase(transformedData);
